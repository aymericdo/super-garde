...
migrate((app) => {
  ...
  // update collection data
  unmarshal({
    "listRule": "@request.auth.id != \"\" && (@request.auth.role = \"god\" || @request.auth.role = \"assistant\" || student = '' || isOnMarket = true || student.user = @request.auth.id || (@request.query.filter ~ 'hospital=' ))"
  }, collection)
