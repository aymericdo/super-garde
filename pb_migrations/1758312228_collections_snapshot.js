...
migrate((app) => {
  const snapshot = [
    ...
    },
    {
      ...
      "system": false,
      "type": "base",
      "updateRule": "@request.auth.id != \"\" && firstName = @request.body.firstName && lastName = @request.body.lastName && (@request.auth.role = 'assistant' || @request.auth.role = 'god')",
      "viewRule": "@request.auth.id != \"\""
    },
    {
      "createRule": "@request.auth.id != \"\" && (@request.auth.role = \"god\" || @request.auth.role = \"assistant\")",
      "deleteRule": "@request.auth.id != \"\" && (@request.auth.role = \"god\" || @request.auth.role = \"assistant\")",
      "fields": [
        {
          "autogeneratePattern": "[a-z0-9]{15}",
          "hidden": false,
