...
migrate((db) => {
  ...
  collection.updateRule = "@request.auth.id != \"\" && @request.data.start:isset = false && @request.data.end:isset = false && ((@request.auth.role = \"god\" || @request.auth.role = \"assistant\") || (student.user = @request.auth.id || (student = '' || isOnMarket = true)))"
